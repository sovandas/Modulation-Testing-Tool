function [ofdm_signal_ch1, ofdm_signal_ch2, qam_dco_ch1, qam_dco_ch2, SM_bits] = SM_generate_ofdm_signal_adaptive_modulation(M, P, Frames, Nfft, cp_length, omitted_carriers, pulse_shaping_type, samples_per_symbol, upper_clipping, lower_clipping, roll_off_factor, Max_Constellation_Size,number_of_pilot_frames,frame_eq_mult) 
...
for k=1:Frames
    ...
    %qam_dco_original = [qam_dco_original, zeros(1,omitted_carriers)];
    for l=1:(Nfft-2)/2-omitted_carriers
        if M(l)~=0
            input_dco = floor(2*rand(log2(M(l)),1));
            SM_bit = floor(2*rand(1,1));
            SM_bits = [SM_bits, SM_bit];
            if (SM_bit==1)
                qam_dco_ch1 = [qam_dco_ch1, modulate(modulator{log2(M(l))},input_dco)/power_rescale(log2(M(l)))*sqrt(P(l))];
                qam_dco_ch2 = [qam_dco_ch2, 0];
            else
                qam_dco_ch1 = [qam_dco_ch1, 0];
                qam_dco_ch2 = [qam_dco_ch2, modulate(modulator{log2(M(l))},input_dco)/power_rescale(log2(M(l)))*sqrt(P(l))];
            end
            %qam_dco_original = [qam_dco_original, modulate(modulator{log2(M(l))},input_dco)/power_rescale(log2(M(l)))];% Inserting zeros in place of the omitted carriers. Carriers are omitted due to the baseline wander. 
        else
            qam_dco_ch1 = [qam_dco_ch1,0];
            qam_dco_ch2 = [qam_dco_ch2,0];
        end
...
dco_ofdm_ch1=SM_ofdm_modulate_real(qam_dco_ch1,Nfft,cp_length);
dco_ofdm_ch2=SM_ofdm_modulate_real(qam_dco_ch2,Nfft,cp_length);
